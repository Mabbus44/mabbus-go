...
function getArg(argsString: string, arg: string): string {
  const argsSplit: string[] = argsString.split(/[=&]+/);
  for (let i: number = 0; i < argsSplit.length; i += 2) if (argsSplit[i] === arg) return argsSplit[i + 1];
  return '';
}

async function httpGet(req, res) {
  const splittedURL: string[] = req.url.split('?', 2);
  const adress: string = splittedURL[0];
  const args: string = typeof splittedURL[1] === 'string' ? splittedURL[1] : '';
  switch (adress) {
    case '/challenge':
    ...
    case '/main':
      ...
      break;
    case '/play': {
      const playerIds: number[] = await uh.getPlayerIds(+getArg(args, 'id'));
      const playerColor: number = uh.getUserColorFromList(playerIds, uh.getUserId(req.session));
...
async function httpPost(req, res) {
  const splittedURL: string[] = req.url.split('?', 2);
  const adress = splittedURL[0];
  // const args = splittedURL[1];
  switch (adress) {
    case '/login':
