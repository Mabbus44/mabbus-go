...
let socketExtras: { [key: string]: SocketExtra } = {};
...
async function getMoves(matchId: number, moveId: number = 0): Promise<void> {
  ...
  let moves: { [key: number]: bf.Coords } = {};
...
async function makeMove(matchId: number, x: number, y: number): Promise<void> {
  ...
  let socket = this;
  let userId: number = uh.getUserId(socket.request.session);
  let userColor: number = await uh.getUserColor(matchId, userId);
...
async function endMatch(matchId: number): Promise<void> {
  ...
  let moves = new Array<bf.Coords>(result.rowCount);
  ...
  if (moves[moves.length - 1].x === -2) {
    ...
    for (let [socketId, socketExtra] of Object.entries(socketExtras))
      if (socketExtra.matchId === matchId) io.sockets.sockets.get(socketId).emit("match ended", points);
    return;
  }
  let board: number[][] = bf.movesToBoard(moves);
